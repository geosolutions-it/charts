...
#### Note:In case the K8s Cluster is not set up for dynamic provisioning
We need to apply StorageClass and PersistenceVolume before Chart deployment.

Here is example StorageClass:
...
```bash
kubectl apply -f example-sc.yaml -n <namespace>
```

Note: To change default storage class, you can find information the following that url:
...
Here is example PersistenceVolume:
...
```bash
kubectl apply -f example-pv.yaml -n <namespace>
```
